...
export default function ContestsPage() {
  ...
  if (!contests) {
    return <Loader label="Kontestlar yuklanmoqda..." />;
  }

  return (
    <div className="space-y-6">
      <div>
        <p className="text-xs uppercase tracking-[0.28em] text-slate-500">Jadval</p>
        <h1 className="mt-2 text-3xl font-semibold text-slate-50">Kontestlar</h1>
        <p className="mt-3 text-sm leading-6 text-slate-400">
          Vaqt va masalalar soni ko'rsatilgan kontestlar ro'yxati.
        </p>
      </div>
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        {contests.map((contest) => (
          <article key={contest.id} className="rounded-[32px] border border-slate-800 bg-slate-950/80 p-6 shadow-glow">
            <p className="text-xs uppercase tracking-[0.24em] text-slate-500">Kontest #{contest.id}</p>
